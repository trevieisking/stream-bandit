/* Stream Bandit V5.21.12 — Clean Player Controls TEST
   Goal: stop the 3-volume-control conflict.
   Keeps custom Stream Bandit fullscreen volume overlay as the main volume control.
   Keeps Sound Booster boost selector, but hides duplicate booster-card volume slider when present.
   Keeps native HTML5 controls visible only when needed, then hides them after idle in player focus.
...
var LOCK_MS=3600;
var BOTTOM_ZONE=180;
var VOL_KEY='streambandit_player_volume_v52112';
...
function loadVol(){
  try{
    var n=Number(localStorage.getItem(VOL_KEY)||localStorage.getItem('streambandit_player_volume_v52111')||localStorage.getItem('streambandit_player_volume_v5219'));
...
function addStyle(){
  if(document.getElementById('sb52112Style'))return;
  var st=document.createElement('style');
  st.id='sb52112Style';
  st.textContent='\nhtml.sb52112ControlsHidden,html.sb52112ControlsHidden *{cursor:none!important}html.sb52112ControlsHidden video::-webkit-media-controls{display:none!important}html.sb52112ControlsHidden video::-webkit-media-controls-enclosure{display:none!important}html.sb52112ControlsHidden video::-webkit-media-controls-panel{display:none!important}html.sb52112ControlsHidden video::-webkit-media-controls-play-button{display:none!important}html.sb52112ControlsHidden video::-webkit-media-controls-timeline{display:none!important}html.sb52112ControlsHidden video::-webkit-media-controls-current-time-display{display:none!important}html.sb52112ControlsHidden video::-webkit-media-controls-time-remaining-display{display:none!important}html.sb52112ControlsHidden video::-webkit-media-controls-mute-button{display:none!important}html.sb52112ControlsHidden video::-webkit-media-controls-volume-slider{display:none!important}html.sb52112ControlsHidden video::-webkit-media-controls-fullscreen-button{display:none!important}.sb52112VolumePanel{position:fixed;left:50%;bottom:24px;transform:translateX(-50%);z-index:999999;display:none;align-items:center;gap:10px;min-width:min(540px,calc(100vw - 34px));padding:12px 14px;border-radius:999px;background:rgba(8,12,24,.92);border:1px solid rgba(182,140,255,.30);box-shadow:0 18px 55px rgba(0,0,0,.45);color:#fff;font-weight:950;backdrop-filter:blur(10px)}.sb52112VolumePanel.show{display:flex}.sb52112VolumePanel button{border:0;border-radius:999px;background:linear-gradient(135deg,#ff2d85,#7c3cff);color:#fff;font-weight:1000;padding:9px 12px;cursor:pointer}.sb52112VolumePanel input[type=range]{flex:1;min-width:150px;accent-color:#ff2d85}.sb52112VolumePanel span{min-width:48px;text-align:right;color:#baf7df}.sb52112HideDuplicateBoosterVolume #sb5215PlayerPanel label:has(#sb5215Volume),.sb52112HideDuplicateBoosterVolume #sb5215Volume{display:none!important}.sb52112HideDuplicateBoosterVolume #sb5215PlayerPanel .sb5215Status{margin-top:10px}@media(max-width:640px){.sb52112VolumePanel{bottom:18px;border-radius:22px}.sb52112VolumePanel button{padding:8px 10px}}\n';
...
function makeVolumePanel(){
  ...
  volPanel.id='sb52112VolumePanel';
  volPanel.className='sb52112VolumePanel';
  volPanel.innerHTML='<button id="sb52112Mute" type="button">🔇</button><b>Volume</b><input id="sb52112VolRange" type="range" min="0" max="100" step="1"><span id="sb52112VolPct">100%</span>';
  document.body.appendChild(volPanel);
  var range=volPanel.querySelector('#sb52112VolRange');
  var mute=volPanel.querySelector('#sb52112Mute');
  ...
  function release(e){if(e)e.stopPropagation();pointerDown=false;setTimeout(function(){sliderActive=false;},500);lockUntil=Date.now()+LOCK_MS;applyVolume();}
...
function syncVolumePanel(){var p=makeVolumePanel();var pct=Math.round(savedVolume*100);var range=p.querySelector('#sb52112VolRange');var label=p.querySelector('#sb52112VolPct');var mute=p.querySelector('#sb52112Mute');if(range&&document.activeElement!==range)range.value=String(pct);if(label)label.textContent=pct+'%';if(mute)mute.textContent=pct===0?'🔈':'🔇';}
...
function showControls(keepOverlay){
  document.documentElement.classList.remove('sb52112ControlsHidden');
...
function forceHideControls(){
  ...
  document.documentElement.classList.add('sb52112ControlsHidden');
...
function bindVideo(v){
  if(!v||v.dataset.sb52112Bound)return;
  v.dataset.sb52112Bound='1';
  applyVolume();
...
function cleanDuplicateVolumeUi(){
  document.documentElement.classList.add('sb52112HideDuplicateBoosterVolume');
  // Extra fallback for browsers without :has(): hide the label containing the fixed-booster volume range.
  var vol=document.getElementById('sb5215Volume');
  if(vol){var lab=vol.closest('label');if(lab)lab.style.display='none';vol.style.display='none';}
  // Remove old toast hints from previous tests.
  ['sb5215FsHint','sb5216Hint','sb5217Hint','sb5218Hint','sb5219Hint','sb52110Hint','sb52111Hint'].forEach(function(id){var x=document.getElementById(id);if(x)try{x.remove();}catch(e){}});
}
function loop(){
  addStyle();cleanDuplicateVolumeUi();videos().forEach(bindVideo);
  var v=activeVideo();
  if(!v||!isWatchLike()){showControls(false);if(volPanel)volPanel.classList.remove('show');return;}
  applyVolume();
  if(!isPlayerFocus()){showControls(false);if(volPanel)volPanel.classList.remove('show');return;}
...
['fullscreenchange','webkitfullscreenchange','msfullscreenchange'].forEach(function(ev){document.addEventListener(ev,function(){lastMove=Date.now();lockUntil=Date.now()+1000;showControls(true);},true);});
