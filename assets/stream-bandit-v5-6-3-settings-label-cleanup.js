/* Stream Bandit V5.11.1 — Settings Label Cleanup
   Visual text cleanup for the Settings page only.
   Main version peacekeeper controls browser title/sidebar badge.
   No Supabase saves, Mux, player, storage upload, logo upload, database or menu routing changes. */
(function(){
'use strict';

var VERSION='V5.11.1';
var CLEAN_TITLE='V5.11.1 Stable Settings + Tabs';
var CLEAN_SHORT='V5.11.1 Stable';
...
function replaceOldText(s){
  return String(s||'')
    ...
    .replace(/StreamBanditPrototypev5\.4\.2-details-tag-display-hotfix-page-polish/g,'StreamBanditPrototypev5.11.1-stable-manager-tabs')
    .replace(/V5\.4\.2 Details Tag Display Hotfix/g,CLEAN_SHORT)
    .replace(/V5\.6\.1 Stable checkpoint/g,'V5.11.1 Stable checkpoint')
    .replace(/V5\.6\.3 Stable Settings \+ Branding/g,CLEAN_TITLE)
    .replace(/V5\.6\.3 Stable/g,CLEAN_SHORT)
...
function addSettingsBadge(){
  ...
  var m=main();
  if(!m)return;
  var all=Array.prototype.slice.call(m.querySelectorAll('.sb563SettingsBadge'));
  all.slice(1).forEach(function(x){try{x.remove();}catch(e){}});
  var existing=all[0];
  if(existing){
    var b=existing.querySelector('b');
    if(b)b.textContent=CLEAN_TITLE;
    var s=existing.querySelector('span');
    if(s)s.textContent='Settings labels cleaned. Existing branding, logo upload and save buttons are unchanged.';
    return;
  }
...
function cleanLocalLabels(){
  try{
    for(var i=0;i<localStorage.length;i++){
      ...
      if(!raw)continue;
      if(raw.indexOf('V5.4.2')<0&&raw.indexOf('V5.6.3')<0&&raw.indexOf('details-tag-display-hotfix')<0&&raw.indexOf('V5.6.1 Stable checkpoint')<0)continue;
