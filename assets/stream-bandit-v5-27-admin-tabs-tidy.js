/* Stream Bandit V5.27.3 — Admin Real Tabs Tidy TEST
   Test route only. Live is untouched.
   Old Admin tabs scroll. This creates a real tab controller and hides non-selected Admin sections.
...
function style(){
 ...
 st.textContent='body.sb527Admin .sb527Off{display:none!important}body.sb527Admin .tabs{display:none!important}.sb527RealTabs{display:flex;gap:10px;flex-wrap:wrap;margin:12px 0 14px;padding:10px;border-radius:22px;background:rgba(4,7,14,.76);border:1px solid rgba(255,255,255,.08);position:sticky;top:0;z-index:50;backdrop-filter:blur(10px)}.sb527RealTabs button{border:0;border-radius:999px;padding:12px 17px;background:rgba(68,72,107,.94);color:#fff;font-weight:1000;cursor:pointer}.sb527RealTabs button.active{background:linear-gradient(135deg,var(--sb-brand-accent-1,#ff2d55),var(--sb-brand-accent-2,#7c3cff))!important;box-shadow:0 14px 34px rgba(255,45,133,.24)}#sb527Note{margin:10px 0 14px;padding:12px 14px;border-radius:16px;background:rgba(34,211,166,.12);border:1px solid rgba(34,211,166,.25);color:#baf7df;font-weight:800}.sb527Pill{display:inline-block;margin-left:8px;padding:3px 8px;border-radius:999px;background:rgba(255,255,255,.08)}.sb527Empty{border:1px dashed rgba(255,255,255,.15);border-radius:22px;padding:24px;color:var(--muted,#a9afc3);text-align:center;margin:12px 0}';
 document.head.appendChild(st);
}
function oldTabs(){return Array.from(app().querySelectorAll('.tabs')).find(function(x){return /overview|rows|edit movie|media|mux|genres|tags|cast|crew|safety/i.test(tx(x));});}
function makeTabs(){
 var ot=oldTabs(); if(!ot)return null;
 ...
 if(!tabs){
   ...
   ot.parentNode.insertBefore(tabs,ot.nextSibling);
 }
 return tabs;
}
...
function topCards(){
 var ot=oldTabs(); if(!ot)return [];
 var parent=ot.parentNode;
 return Array.from(parent.children).filter(function(el){
   if(el===ot||el.id==='sb527RealTabs'||el.id==='sb527Note'||el.classList.contains('sb527RealTabs'))return false;
...
function show(k){
 ...
 var n=note(); if(n)n.innerHTML='✅ V5.27.3 Admin real tabs: showing <b>'+k+'</b> only.<span class="sb527Pill">'+shown+' shown</span><span class="sb527Pill">No writes</span>'+(shown?'':'<div class="sb527Empty">No sections found for this tab yet.</div>');
}
