/* Stream Bandit V5.24.1 — Add Quality Tools tab to full Tools page TEST
   Loads on top of tools-v5-20-2.html in a test route only.
...
function updateHeader(){
 document.title='Stream Bandit Tools V5.24.1 Quality Tools Merge Test';
 var pill=qs('.pill'); if(pill) pill.innerHTML='<span class="dot"></span> V5.24.1 Full Tools + Quality Tools Test';
 var hero=qs('.hero p'); if(hero) hero.textContent='V5.24.1 keeps the full V5.20.2 Tools page and adds the read-only Quality Tools audit as a normal Tools tab. V5.20.2 remains the fallback until this merge test passes.';
 var mini=qsa('.hero .mini');
 if(mini[0]) mini[0].innerHTML='<b>Live fallback kept</b><span>V5.20.2 remains the fallback while this V5.24.1 merge test is checked.</span>';
...
function addPanel(){
 ...
 qs('#sb5241CopyMoveSafety').onclick=function(){copyText(lastMoveSafety,'Move safety report copied')};
}
...
async function runMoveSafety(){var list=qs('#sb5241SafetyList');list.innerHTML='';var lines=['Stream Bandit V5.24.1 full Tools + Quality Tools merge safety report'];lines.push(addStatus('✅','V5.24.1 page loaded','Full Tools page with Quality Tools tab is running.'));lines.push(addStatus(location.pathname.indexOf('tools-v5-24-1-full-quality-test.html')>-1?'✅':'⚠️','Direct V5.24.1 test page',location.pathname));var paths=['index.html','tools-v5-20-2.html','tools-v5-24-quality-tools-test.html','tools-v5-24-1-full-quality-test.html','assets/stream-bandit-v5-24-1-tools-quality-tab.js','assets/stream-bandit.css','assets/stream-bandit-v5-5-polish.css','assets/stream-bandit-logo.png'];for(var i=0;i<paths.length;i++){var r=await checkUrl(paths[i]);lines.push(addStatus(r.ok?'✅':'❌',paths[i],String(r.status)))}try{var rows=await getMovies();lines.push(addStatus('✅','Supabase read: sb_movies',rows.length+' rows visible to this key'))}catch(e){lines.push(addStatus('⚠️','Supabase read: sb_movies',e.message))}lines.push(addStatus('✅','No write actions used','This merge test did not insert, update, delete, upload or run quick fixes.'));lastMoveSafety=lines.join('\n');toast('Move safety checks finished')}
function init(){addCss();updateHeader();addTab();addPanel();if(location.hash==='#quality')show('quality')}
