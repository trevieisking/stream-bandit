...
function enhance(){const video=document.querySelector('#frame video, video');if(!video)return false;seekWhenReady(video);['timeupdate','pause','ended'].forEach(ev=>video.addEventListener(ev,()=>saveProgress(video)));setInterval(()=>saveProgress(video),5000);const badge=document.querySelector('.badge');if(badge)badge.textContent='V6.73 Player Resume TEST';const note=document.querySelector('.hero p');if(note)note.textContent='Player now saves local Continue Watching progress and resumes from exact timestamp when opened with ?t=seconds.';window.StreamBanditProgressV673={version:VERSION,store:STORE,read:readStore,save:()=>saveProgress(video)};return true;}
