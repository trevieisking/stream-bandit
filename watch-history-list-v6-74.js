/* Stream Bandit V6.74.1 Watch History List Helper
   Reads local browser watch-history entries and renders real recent rows.
   Fix: counts matched displayable rows only, so Overview and History Cards agree.
   No Supabase writes, no movie/admin/storage/billing/live actions. */
(function(){
'use strict';
const VERSION='V6.74.1 Watch History Real Tracking TEST';
...
function historyRows(){const hist=read(HISTORY_STORE),prog=read(PROGRESS_STORE),rows=Object.values(hist).filter(x=>x&&x.id&&x.id!=='latest');rows.forEach(r=>{const p=prog[String(r.id)];if(p&&p.currentTime)r.currentTime=p.currentTime;if(p&&p.duration)r.duration=p.duration});return rows.sort((a,b)=>String(b.lastWatchedAt||'').localeCompare(String(a.lastWatchedAt||'')))}
function byIdMap(){const by={};movies.forEach(m=>by[String(m.id)]=m);return by}
function matchedRows(){const by=byIdMap();return historyRows().map(r=>({row:r,movie:by[String(r.id)]||null})).filter(x=>x.movie)}
function card(item,mode){const row=item.row,m=item.movie;if(!m)return'';const id=encodeURIComponent(m.id||''),img=poster(m),u=firstUrl(m),t=sec(row.currentTime),gs=arr(m.genres).slice(0,2),playHref=PLAYER+'?id='+id+(t?'&t='+t:'');return'<article class="movie"><div class="thumb">'+(img?'<img src="'+esc(img)+'" loading="lazy" alt="'+esc(m.title||'Artwork')+'">':'No artwork')+'</div><div class="body"><h3>'+esc(m.title||'Untitled')+'</h3><p>'+esc(m.description||'No description yet.')+'</p><div><span class="pill">'+esc(mode)+'</span><span class="pill">Last: '+esc(row.lastWatchedAt?new Date(row.lastWatchedAt).toLocaleString():'recent')+'</span>'+(t?'<span class="pill">Resume '+fmt(t)+'</span>':'')+'<span class="pill">'+esc(sourceType(m))+'</span>'+gs.map(g=>'<span class="pill">'+esc(g)+'</span>').join('')+'</div><div class="actions"><a class="btn small primary" href="'+DETAILS+'?id='+id+'">Details</a>'+(u?'<a class="btn small hot" href="'+PLAYER+'?id='+id+'">Play</a>':'')+(u&&t?'<a class="btn small hot" href="'+playHref+'">Resume '+fmt(t)+'</a>':'')+'</div></div></article>'}
function fallbackItems(){return movies.filter(firstUrl).slice(0,6).map(m=>({row:{id:String(m.id),currentTime:0,lastWatchedAt:'',fallback:true},movie:m}));}
function render(){updateText();const raw=historyRows(),matched=matchedRows(),shown=matched.length?matched:fallbackItems();const html=shown.map(r=>card(r,matched.length?'History':'Link Test')).join('')||'<div class="note">No Watch History rows found yet.</div>';if($('stats'))$('stats').innerHTML=stat('Raw local entries',raw.length)+stat('Matched rows',matched.length)+stat('Supabase movies',movies.length)+stat('Store','local only');if($('historyGrid'))$('historyGrid').innerHTML=matched.length?html:'<div class="note">No matched Watch History rows found yet. Play a movie opened from a card/Details link so it has a real movie ID.</div>';if($('progressGrid'))$('progressGrid').innerHTML=matched.length?html:'<div class="note">No matched progress rows found yet. Play a movie for 20–30 seconds, then return here.</div>';setStatus(matched.length?'Watch History loaded '+matched.length+' matched real row(s).':(raw.length?'Found '+raw.length+' raw local entrie(s), but none match movie rows yet. Play again after this hotfix.':'No local watch history yet. Play a movie for 20–30 seconds, then return here.'));}
async function load(){try{setStatus('Loading V6.74.1 Watch History...');if(!window.supabase)throw Error('Supabase SDK did not load');const cfg=await readConfig();sb=supabase.createClient(cfg.url,cfg.key);const r=await sb.from('sb_movies').select('*').order('created_at',{ascending:false}).limit(250);if(r.error)throw r.error;movies=(r.data||[]).filter(m=>String(m.status||'published')!=='archived');render()}catch(e){setStatus('Watch History V6.74.1 failed: '+(e.message||e),true)}}
