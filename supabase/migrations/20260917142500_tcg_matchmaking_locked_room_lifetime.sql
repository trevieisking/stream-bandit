...
-- G0R-07: queue expiry applies only while a matchmaking room is waiting.
-- Once a room is locked or in_match, its original queue expiry must not hide
-- the room from either matched player's idempotent retry/poll path.
...
begin
  ...
  -- Queue expiry is meaningful only while waiting. A locked/in_match room is
  -- already a real match container and stays discoverable after that timestamp.
