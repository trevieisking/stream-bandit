...
begin
  ...
  where not x.basic_essence and x.qty > 3;
  ...
  if v_overcopies > 0 then v_errors := v_errors || jsonb_build_array('deck_exceeds_three_copy_gameplay_name_limit'); end if;
...
revoke all on function public.tcg_server_validate_deck(uuid,uuid) from public, anon, authenticated;
grant execute on function public.tcg_server_validate_deck(uuid,uuid) to service_role;
