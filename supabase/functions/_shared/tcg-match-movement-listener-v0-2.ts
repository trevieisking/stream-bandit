...
function inspectDeckTop(state: Record<string, unknown>, candidate: Candidate, event: RuntimeV02MovementListenerEvent, ownerSeat: 1 | 2, count: number, exactMinimum: number): CardRef[] {
  ...
  if (cards.length) {
    recordRuntimeV02HiddenInformationView(state, candidate.seat, "deck_top", {
      action_kind: "movement_listener",
...
function executeStep(state: Record<string, unknown>, continuation: Continuation, candidate: Candidate, event: RuntimeV02MovementListenerEvent, step: Record<string, unknown>): "continue" | "choice" {
  ...
  if (op === "LOOK_TOP") { const owner = playerForToken(candidate, step.player); const count = Math.max(0, Number(step.count || 0)); continuation.vars[String(step.as || "looked")] = inspectDeckTop(state, candidate, event, owner, count, 0); continuation.step_cursor++; return "continue"; }
  if (op === "INSPECT_ZONE") { if (String(step.zone || "") !== "deck_top" || String(step.return_policy || "") !== "same_position" || String(step.visibility || "") !== "controller_private") throw new Error("tcg_v0_2_movement_listener_inspection_shape_unsupported"); const owner = playerForToken(candidate, step.player); const wanted = range(step.selection); const count = wanted.max; continuation.vars[String(step.as || "looked")] = inspectDeckTop(state, candidate, event, owner, count, wanted.min); continuation.step_cursor++; return "continue"; }
