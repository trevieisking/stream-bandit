...
/**
 * Owner #28 domain-event adapter for canonical owner #34 defeat records.
 *
 * Defeat physically removes the subject before listeners run, so this adapter
 * freezes the departed Creature identity into the ordinary Event Listener envelope
 * instead of trying to rediscover it on the battlefield. Candidate collection,
 * limits, replay receipts, choices and effect execution remain owned by the normal
 * Event Listener engine. No card or element names are interpreted here.
 */
export function runtimeV02AdaptDefeatEventForListener(
  ...
  const ownerSeat = seat(raw.owner_seat, "tcg_v0_2_defeat_listener_owner_invalid");
  const sourceController = raw.source_controller_seat == null
    ? undefined
    : seat(raw.source_controller_seat, "tcg_v0_2_defeat_listener_source_controller_invalid");
  ...
  if (where === "reserve" && (!Number.isInteger(raw.index) || Number(raw.index) < 0 || Number(raw.index) > 3)) {
    throw new Error("tcg_v0_2_defeat_listener_reserve_index_invalid");
  }
  const phase = typeof state.phase === "string" && state.phase.trim()
    ? state.phase.trim()
    : "resolution";
  const eventId = requiredString(raw.event_id, "tcg_v0_2_defeat_listener_event_id_required");
  const sourceActionId = raw.source_action_id == null
    ? eventId
    : requiredString(raw.source_action_id, "tcg_v0_2_defeat_listener_source_action_id_invalid");
  const actionKind = raw.action_kind == null
    ? "defeat"
    : requiredString(raw.action_kind, "tcg_v0_2_defeat_listener_action_kind_invalid");
  const sourceCardUid = raw.source_card_uid == null
    ? null
    : requiredString(raw.source_card_uid, "tcg_v0_2_defeat_listener_source_card_uid_invalid");

  return {
    event_id: eventId,
    event: "creature_defeated",
    subject_uid: requiredString(raw.creature_uid, "tcg_v0_2_defeat_listener_subject_uid_required"),
    subject_card_id: requiredString(raw.card_id, "tcg_v0_2_defeat_listener_subject_card_id_required"),
...
export function runtimeV02AdaptDefeatEventsForListener(
  ...
  if (!Array.isArray(events)) throw new Error("tcg_v0_2_defeat_listener_events_required");
  return events.map((event) => runtimeV02AdaptDefeatEventForListener(state, event));
}

/**
 * Canonical #34 -> #28 bridge. It deliberately delegates the adapted events back
 * into the existing Event Listener continuation so there is still only one listener
 * execution/replay/choice owner.
 */
