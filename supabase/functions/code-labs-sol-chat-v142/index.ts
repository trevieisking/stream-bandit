...
const VERSION = "Code Labs Sol Chat V145";
...
function safeArray(value: unknown, maxItems: number, maxText: number) {
  ...
  return value.slice(0, maxItems).map((entry) => {
    ...
    for (const [key, item] of Object.entries(entry as Record<string, unknown>)) out[key] = typeof item === "string" ? text(item, maxText) : item;
    return out;
  });
}
function compactSections(value: unknown, maxItems: number, maxText: number, fieldText: number, actionText: number) {
  if (!Array.isArray(value)) return [];
  return value.slice(0, maxItems).map((entry) => {
    if (!entry || typeof entry !== "object") return entry;
    const out: Record<string, unknown> = {};
    for (const [key, item] of Object.entries(entry as Record<string, unknown>)) {
      if (key === "fields") out[key] = safeArray(item, 120, fieldText);
      else if (key === "actions") out[key] = safeArray(item, 80, actionText);
      else out[key] = typeof item === "string" ? text(item, maxText) : item;
    }
    return out;
  });
}
function compact(value: unknown) {
  ...
  return {
    ...
    counts: p.counts || {}, sections: compactSections(p.sections, 80, 4000, 4000, 1500), fields: safeArray(p.fields, 240, 12000), actions: safeArray(p.actions, 160, 4000),
...
function compactReadPacket(value: unknown) {
  ...
  return {
    ...
    sections: compactSections(p.sections, 80, 1500, 1500, 1000), fields: safeArray(p.fields, 240, 4000), actions: safeArray(p.actions, 160, 1500),
  };
}
...
async function openai(body: Record<string, unknown>, stage: string) {
  ...
  try {
    response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${OPENAI_API_KEY}`, "Content-Type": "application/json" }, body: JSON.stringify(body) });
  } catch (error) {
    const detail = text((error as Error).message || error, 1000);
    console.error("Code Labs Sol upstream network failure", { stage, detail });
    throw new Error(`Code Labs Sol could not complete the ${stage} request. Retry shortly; the administrator can inspect the server logs if it continues.`);
  }
  ...
  if (!response.ok) {
    ...
    console.error("Code Labs Sol upstream failure", { stage, status: response.status, code, detail });
    if (response.status === 401 || response.status === 403 || /auth|api[_ -]?key|credential|invalid.*key/i.test(code)) {
      throw new Error("Code Labs Sol could not verify its AI service configuration. Ask the administrator to check the server settings.");
    }
    throw new Error(`Code Labs Sol could not complete the ${stage} request. Retry shortly; the administrator can inspect the server logs if it continues.`);
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!OPENAI_API_KEY) return json(req, { ok: false, error: "Code Labs Sol is waiting for its server-side OpenAI key." }, 200);
    ...
    if (action === "tool_outputs") {
      ...
      if (!previousResponseId) return json(req, { ok: false, error: "Previous Sol response is missing. Start a new Sol chat and retry.", version: VERSION, stage }, 200);
      const outputs = Array.isArray(body.tool_outputs) ? body.tool_outputs : [];
      if (!outputs.length) return json(req, { ok: false, error: "Tool output is missing.", version: VERSION, stage }, 200);
      ...
      if (!message.trim()) return json(req, { ok: false, error: "Message required" }, 200);
    ...
    const status = /sign-in|approved|authentication/i.test(message) ? 401 : 200;
