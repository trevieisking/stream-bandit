...
const baselinePattern = /^(\d{14})_stream_bandit_replay_foundation\.sql$/;
...
function baselineFile() {
  const matches = fs.readdirSync(migrations).filter((name) => baselinePattern.test(name));
  assert.equal(matches.length, 1, 'exactly one replay-foundation migration must exist');
  const match = matches[0].match(baselinePattern);
  assert.ok(match, 'baseline filename must contain a 14-digit migration version');
  assert.ok(match[1] < firstRecordedVersion, 'baseline must sort before the first recorded migration');
  return path.join(migrations, matches[0]);
}
