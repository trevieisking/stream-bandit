...
import { scanCodeLabsRepository } from "./scan-labs.ts";
...
function tools() {
  return [
    ...
    { name: "scan_code_labs_repository", title: "Scan Repository with Scan Labs", description: "Run Scan Labs as an early, unnumbered, whole-repository read-only reconnaissance tool at an exact owner-authorized GitHub commit. It maps areas, pages, dependencies, environment-variable calls, database and RPC patterns, and browser state while redacting credential-shaped values. Results are paginated by section. It cannot change workflow state, repair source, run Code God, queue Writer, commit, merge, deploy, or decide promotion.", inputSchema: { type: "object", properties: { repo: { type: "string", description: "Owner-authorized repository in owner/name form." }, ref: { type: "string", description: "Optional branch, tag, or exact commit. Defaults to the verified repository's default branch." }, section: { type: "string", enum: ["summary", "areas", "pages", "dependencies", "environment", "database", "state", "findings", "all"], description: "Report section to return after the whole repository is scanned. Defaults to summary." }, offset: { type: "number", minimum: 0, description: "Zero-based section offset. Defaults to 0." }, limit: { type: "number", minimum: 1, maximum: 500, description: "Maximum returned rows per section. Defaults to 200." } }, required: ["repo"] }, outputSchema: resultSchema, annotations: read },
    ...
    { name: "undo_code_labs_action", title: "Undo Code Labs Action", description: "Restore an eligible in-place Code Labs write from its receipt after matching the current workspace version.", inputSchema: { type: "object", properties: { receipt_id: { type: "string" }, expected_state_version: expected }, required: ["receipt_id", "expected_state_version"] }, outputSchema: resultSchema, annotations: destructiveWrite },
...
async function call(b: any, name: string, args: Row) {
  ...
  if (name === "scan_code_labs_repository") return scanCodeLabsRepository(b, args);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (body.jsonrpc === "2.0") {
      if (body.method === "initialize") return rpc(id, { protocolVersion: "2025-06-18", capabilities: { tools: { listChanged: true } }, serverInfo: { name: "code-labs-mcp-stub", version: VERSION }, instructions: "Use Code Labs V104 server tools directly. Scan Labs is the optional early, unnumbered, whole-repository read-only reconnaissance tool and never advances or owns workflow state. The reviewed repair route remains File Lab, CG Repair Lab, Code God, then GitHub Writer. CG Repair Lab is Code Labs Pro, owner-scoped, read-only, and value-redacting. The private owner gallery is available only through opaque read-only references and deliberate selected-image reads. No browser pairing, page session, page fingerprint, or live-tab control is used. Read the workspace before writes, supply its current state_version, inspect receipts, and keep GitHub changes branch and pull-request only." });
