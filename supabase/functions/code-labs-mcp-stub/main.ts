...
import { createCheckpoint, getWorkspace, listActions, listRecords, readCurrentFile, readReceipt, runAction, saveCandidate, selectRecord, undoAction, updateCurrentFile, updateJob, updatePacket, updateProject, updateTest } from "./guarded-workspace.ts";
...
function tools() {
  return [
    ...
    { name: "run_code_labs_action", title: "Run Code Labs Action", description: "Run one strict server-side Code Labs action ID. State-changing actions require the current workspace version; this never clicks a browser page.", inputSchema: { type: "object", properties: { action: { type: "string" }, record_id: { type: "string" }, expected_state_version: expected, confirmed: { type: "boolean" }, label: { type: "string" }, note: { type: "string" }, fields, candidate_code: { type: "string" } }, required: ["action"] }, outputSchema: resultSchema, annotations: destructiveWrite },
