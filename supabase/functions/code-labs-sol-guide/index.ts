...
const VERSION = "Code Labs Sol Guide V224";
const API_KEY = Deno.env.get("OPENAI_API_KEY") || "";
const CONFIGURED_MODEL = Deno.env.get("OPENAI_MODEL") || "gpt-5.6-terra";
...
function isModelAccessFailure(failure: UpstreamFailure) {
  return failure.status === 403 || failure.status === 404 || /model|not_found|unsupported|permission/i.test(failure.code + " " + failure.message);
}
...
function publicFailure(failure: UpstreamFailure) {
  ...
  if (failure.status === 401 || /invalid_api_key|authentication/i.test(combined)) {
    return "Sol's server API credential is invalid or inactive.";
  }
  if (failure.status === 429 && /insufficient_quota|quota|billing|credits|spend/i.test(combined)) {
    return "Sol's OpenAI API quota or spending limit has been reached.";
  }
  if (failure.status === 429) {
    return "Sol is temporarily rate limited. Please wait a moment and try again.";
  }
  if (isModelAccessFailure(failure)) {
    return "Sol's configured model is not available to this OpenAI project, and the safe fallback could not respond.";
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!attempt.ok && CONFIGURED_MODEL !== FALLBACK_MODEL && isModelAccessFailure(attempt.failure)) {
      usedFallback = true;
