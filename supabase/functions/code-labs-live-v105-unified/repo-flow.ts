...
export async function prepareGithubWriter(b: Binding, args: Row) {
  ...
  if (review.outcome !== "PASS") {
    throw new Error("Code God PASS is required before V105 GitHub operation preparation.");
  }
  if (
  ...
  const request = {
    ...
    pr_body: String(
      args.fields?.pr_body || "Prepared by Code Labs V105 unified after Code God PASS.",
    ).slice(0, 20000),
  ...
  const rows = await rest("code_labs_write_requests", {
    ...
    body: JSON.stringify({
      ...
      requested_source: "code_labs_v105_unified",
      repo: request.repo,
      path: request.path,
      ...
      safety_note: "Code God PASS recorded before unified V105 execution.",
    }),
  });
  const queued = rows?.[0];
  ...
  return {
    ...
    queued: marker,
    next_tool: "Code Labs V105 unified GitHub operation",
    file_id: file.id,
    receipt: await receipt(
      b.owner_id,
