...
async function opaqueReference(ownerId: string, objectPath: string) {
  ...
  if (!secret) {
    throw new Error("Code Labs gallery reference key is unavailable.");
  }
...
function activeEntitlement(row: Record<string, unknown> | null) {
  if (!row || row.plan_key !== "pro" || row.status !== "active") {
    return false;
  }
  const now = Date.now();
...
async function requireOwnerPro(ownerId: string) {
  const [owners, entitlements] = await Promise.all([
    rest(
      "code_labs_owners?select=user_id&user_id=eq." +
...
function cleanReference(value: unknown) {
  ...
  if (!/^img_[a-f0-9]{64}$/.test(reference)) {
    throw new Error("A valid opaque gallery image reference is required.");
  }
  return reference;
}
...
function mediaType(row: StorageObject) {
  const value = String(
    row.metadata?.mimetype || row.metadata?.contentType || "",
  ).toLowerCase();
...
async function listObjects(ownerId: string) {
  ...
  if (!key) {
    throw new Error("Code Labs gallery service access is unavailable.");
  }
  ...
  if (!response.ok || !Array.isArray(data)) {
    throw new Error("Private gallery listing was blocked.");
  }
  return data
    .filter((row: StorageObject) =>
      row && row.name && row.name !== ".emptyFolderPlaceholder"
    )
    .slice(0, MAX_ITEMS);
}
...
async function indexedObjects(ownerId: string) {
  ...
  return Promise.all(
    rows.map(async (row: StorageObject) => {
      const objectPath = ownerId + "/" + String(row.name);
      return {
        row,
        objectPath,
        reference: await opaqueReference(ownerId, objectPath),
      };
    }),
  );
}
...
export async function listOwnerGalleryReferences(binding: Binding) {
  ...
  return {
    ok: true,
    version: "Code Labs V105 unified owner gallery read bridge V1",
    count: images.length,
    images,
    read_only: true,
...
export async function readOwnerGalleryImage(
  binding: Binding,
  ...
  if (!selected) {
    throw new Error("The selected gallery image reference is unavailable.");
  }

  const key = serviceKey();
  const encodedPath = selected.objectPath.split("/")
    .map(encodeURIComponent)
    .join("/");
  const response = await fetch(
    SUPABASE_URL + "/storage/v1/object/authenticated/" + BUCKET + "/" +
      encodedPath,
    {
      headers: {
        apikey: key,
  ...
  if (!response.ok) {
    throw new Error("The selected private gallery image could not be read.");
  }
  const contentLength = Number(
    response.headers.get("content-length") || 0,
  );
  if (contentLength > MAX_BYTES) {
    throw new Error("The selected gallery image exceeds the read limit.");
  }
  const bytes = new Uint8Array(await response.arrayBuffer());
  if (!bytes.length || bytes.length > MAX_BYTES) {
    throw new Error(
      "The selected gallery image is empty or exceeds the read limit.",
    );
  }
  const headerType = String(response.headers.get("content-type") || "")
    .split(";")[0]
    .toLowerCase();
  const type = ALLOWED_TYPES.has(headerType)
    ? headerType
    : mediaType(selected.row);
  if (!ALLOWED_TYPES.has(type)) {
    throw new Error("The selected object is not an allowed image type.");
  }
  let binary = "";
  const chunk = 0x8000;
