...
export const BASE = SUPABASE_URL + "/functions/v1/code-labs-live-v105";
export const SCOPE = "code_labs.read code_labs.write";
export const CLAIM = "code-labs-v105";
...
async function configuredOwnerId() {
  ...
  if (rows.length > 1) throw new Error("Code Labs V105 requires exactly one configured owner.");
...
export async function exactSession(owner_id: string, session_id: string) {
  ...
  if (!session) throw new Error("No active V105 Code Labs page is available. Open one Code Labs page and keep it visible.");
  if (!session.control_expires_at || new Date(session.control_expires_at).getTime() < Date.now()) throw new Error("The active V105 Code Labs page session expired. Refresh the page.");
  return session;
}
export async function activeSession(owner_id: string): Promise<Binding> {
  ...
  if (!Array.isArray(rows) || !rows[0]) throw new Error("No active V105 Code Labs page is available. Open one Code Labs page and keep it visible.");
...
async function issue(owner_id: string) {
  ...
  if (!owner_id || owner_id !== configured) throw new Error("OAuth owner is not approved for Code Labs V105.");
