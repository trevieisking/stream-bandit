...
function findCreature(state: any, ref: CreatureRef | null | undefined) {
  ...
  for (const item of allCreatures(player)) {
    if ((item.cr.stack || []).some((inst: Inst) => inst.uid === ref.anchor_uid)) {
      return { ...item, seat: ref.seat, player };
    }
  }
  return null;
}
...
function choiceBounds(raw: any, optionCount: number, hiddenSearchCanFail = false) {
  const range = countRange(raw);
  if (!hiddenSearchCanFail && optionCount < range.min) throw new Error("required_effect_choice_unavailable");
  return {
    min: hiddenSearchCanFail ? Math.min(range.min, optionCount) : range.min,
    max: Math.min(range.max, optionCount),
  };
}
...
function creatureOptions(state: any, ownerSeat: number, controller: unknown, zone: unknown, filters: any) {
  ...
  for (const item of allCreatures(player)) {
    ...
    const d = topDef(item.cr, state) || {};
    out.push({
      id: `creature:${seat}:${ref.anchor_uid}`,
      label: String(d.name || "Creature"),
      data: ref,
    });
  }
  return out;
...
function cardOptions(state: any, cards: Inst[], filters: any, ownerSeat: number) {
  return cards
    .filter((inst) => matchesCardFilters(state, inst, filters, ownerSeat))
    .map((inst) => ({
      id: `card:${inst.uid}`,
...
function choiceView(choice: PendingChoice | null, viewerSeat: number) {
  ...
  if (Number(choice.seat) !== viewerSeat) {
...
function evaluateWinner(state: any) {
  ...
  if (one > two) {
    state.result = { winner_seat: 1, reasons: reasons["1"], reason: reasons["1"][0] || "complete" };
  } else if (two > one) {
    state.result = { winner_seat: 2, reasons: reasons["2"], reason: reasons["2"][0] || "complete" };
  } else {
...
function selectChoiceOptions(pending: PendingChoice, ids: string[]) {
  ...
  if (pending.mode === "order" && (ids.length !== pending.options.length || ids.length !== pending.max)) {
    throw new Error("order_requires_every_option_exactly_once");
  }
  ...
  while (!state.pending_choice && !state.pending_heal_listener_choice && !state.pending_movement_listener_choice && effect.cursor < effect.steps.length) {
    ...
    if (op === "SELECT_CREATURE") {
      ...
      setPending(state, effect, {
        ...
        mode: "select",
        options,
        context: { apply: "set_var", var_name: String(step.as || "target"), many: bounds.max !== 1 },
      });
      return;
    }
    ...
    if (op === "CHOOSE_PLAYER") {
      ...
      for (const allowed of step.allowed || ["self", "opponent"]) {
        ...
        options.push({
          id: `player:${allowed}`,
          label: allowed === "self" ? "You" : "Opponent",
          data: { player: allowed },
        });
      }
      if (!options.length) throw new Error("required_player_choice_unavailable");
      setPending(state, effect, {
        seat: ownerSeat,
        kind: "choose_player",
      ...
      for (const source of creatures) {
        ...
        for (const essence of source.cr.essence || []) {
          ...
          for (const destination of creatures) {
            ...
            options.push({
              id: `move:${essence.uid}:${destinationRef.anchor_uid}`,
...
function applyPendingChoice(state: any, selected: ChoiceOption[]) {
  ...
  if (apply === "set_var") {
    ...
    const zoneSeat = Number(context.zone_seat);
    if (zoneSeat !== 1 && zoneSeat !== 2) throw new Error("selected_deck_zone_seat_invalid");
    if (String(context.destination) !== "hand") throw new Error("tactic_search_destination_unsupported");
    const player = state.players[String(zoneSeat)];
    if (selected.length > 0) {
      const selectedUids = selected.map((option) => String(option.data.uid));
      let preflight;
      try {
        preflight = runtimeV02PreflightCardZoneTransfer(player.deck as Inst[], player.hand as Inst[], {
          cause: "effect",
          action_kind: "tactic",
          source_action_id: effect.id,
          source_card_uid: effect.source_card.uid,
          source: { controller_seat: zoneSeat as 1 | 2, zone: "deck", owner_card_uid: null },
          destination: { controller_seat: zoneSeat as 1 | 2, zone: "hand", owner_card_uid: null },
          card_uids: selectedUids,
          destination_position: "bottom",
        });
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (message.startsWith("tcg_v0_2_card_zone_selected_card_missing:")) throw new Error("selected_deck_card_missing");
        throw error;
      }
      for (let index = 0; index < selected.length; index += 1) {
        const option = selected[index];
        const current = preflight.cards[index];
        if (current.uid !== String(option.data.uid) || current.card_id !== String(option.data.card_id)) {
          throw new Error("selected_deck_card_changed");
        }
      }
    ...
    const movementEvents: ReturnType<typeof runtimeV02CreateEssenceMovedEvent>[] = [];
    ...
    throw new Error(`unknown_pending_apply:${apply}`);
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!matchId || !nonce || !Number.isInteger(expected) || expected < 0) {
      return json({ ok: false, version: VERSION, error: "match_id_client_nonce_expected_revision_required" }, 400);
    }

    const { data: prior, error: priorError } = await admin
      .from("tcg_match_commands")
      .select("status,result,expected_revision,resolved_at")
      .eq("match_id", matchId)
      .eq("user_id", userId)
    ...
    if (action === "play_tactic") {
      ...
      const result = await commit("play_tactic", {
        seat,
    ...
    if (movementPending) {
      ...
      if (resolved.status === "player_choice_required") {
        if ((resolved.emitted_heal_packet_ids || []).length) throw new Error("tcg_v0_2_tactic_movement_choice_with_prior_heal_not_yet_supported");
    ...
    if (String(body.choice_id || "") && String(body.choice_id) !== pending.id) {
      return json({ ok: false, version: VERSION, error: "stale_choice_id" }, 409);
    }
    ...
    executeUntilChoice(state);
    const result = await commit("resolve_effect_choice", {
      seat,
      kind: pending.kind,
