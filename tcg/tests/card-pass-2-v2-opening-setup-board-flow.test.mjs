...
test('desktop battlefield gives Vanguard and Reserves independent height-bounded rows', () => {
  ...
  assert.match(battle, /\.sb-card-wrap\.is-selected \.sb-card-actions\{[\s\S]*?position:absolute;[\s\S]*?left:calc\(100% \+ 10px\);[\s\S]*?max-height:calc\(100% - 4px\);[\s\S]*?overflow-y:auto/);
