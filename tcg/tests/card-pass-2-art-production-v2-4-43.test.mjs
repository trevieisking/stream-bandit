...
assert.ok(batches.every(x=>["production_blueprint_complete_art_missing","art_complete"].includes(x.status)));
...
assert.ok(cards.every(x=>["approved","missing"].includes(x.artwork_status)));

for(const batch of batches){
  assert.equal(batch.approved_art_count,batch.cards.filter(x=>x.artwork_status==="approved").length,batch.id);
  if(batch.approved_art_count===batch.count) assert.equal(batch.status,"art_complete",batch.id);
}

const intakeById=new Map(intake.cards.map(x=>[x.card_id,x]));
assert.equal(intakeById.size,193);
const printingById=new Map(printing.base_printings.map(x=>[x.card_id,x]));
assert.equal(printingById.size,193);
for(const card of cards){
  const intakeRow=intakeById.get(card.card_id);
  const p=printingById.get(card.card_id);
  assert.ok(intakeRow,card.card_id);
  ...
  assert.equal(intakeRow.expected_asset_path,card.target_path,card.card_id);
  assert.equal(intakeRow.expected_filename,card.target_path.split("/").pop(),card.card_id);
  assert.equal(p.artwork_status,card.artwork_status,card.card_id);
  assert.equal(intakeRow.artwork_status,card.artwork_status==="approved"?"complete":"missing",card.card_id);
  if(card.artwork_status==="approved") assert.ok(fs.existsSync(card.target_path),card.card_id);
}
...
const approved=cards.filter(x=>x.artwork_status==="approved");
assert.equal(ledger.card_art_batches.production_ready_card_count,193);
assert.equal(ledger.card_art_batches.total_base_art_approved,approved.length);
assert.equal(intake.artwork_complete,approved.length);
assert.equal(intake.artwork_missing,193-approved.length);
assert.equal(printing.status_counts.artworks_approved,approved.length);
assert.equal(printing.status_counts.artworks_missing,193-approved.length);
assert.equal(approved.filter(x=>printingById.get(x.card_id).element==="Astral").length,24);
assert.equal(ledger.card_art_batches.set_one_blueprint_complete,true);
assert.equal(ledger.card_art_batches.actual_art_completion,approved.length===193);
assert.equal(ledger.background_production.final_backgrounds_approved,0);

console.log("V2.4.46 Set One art production state: PASS");
