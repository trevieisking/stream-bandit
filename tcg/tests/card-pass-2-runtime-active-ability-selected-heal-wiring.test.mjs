...
test('generic active Ability selected-heal owner stays card-id-free and reuses existing limit and heal-packet owners', () => {
  for (const token of [
    'grove-myceliarch',
    'networked-growth',
    'grove-sporeling',
  ...
  assert.ok(activeChoiceOwner.includes('recordActiveAbilityUse(state, controller, descriptor.ability_id)'));
...
test('Device resolution has one existing server-owned current-turn truth source and the new owner only reads it', () => {
...
test('live match owner deliberately remains on the already-green Reward Ability family until resume-to-play listener wiring exists', () => {
  const useAbility = blockBetween(match, 'if(action==="use_ability")', 'if(action==="play_creature")');
  assert.ok(useAbility.includes('structuredRuntimeActiveAbilityRewardInspection(s,source)'));
  assert.ok(useAbility.includes('runtimeV02CreateActiveAbilityRewardChoice('));
  assert.equal(useAbility.includes('structuredRuntimeActiveAbilitySelectedHeal'), false);
  assert.equal(useAbility.includes('runtimeV02BuildActiveAbilitySelectedHealChoice'), false);
  assert.equal(match.includes('tcg-match-active-ability-selected-heal-v0-2.ts'), false);
  assert.ok(match.includes('active_ability_requires_runtime_owner'));
});

test('Myceliarch attack blocker remains intact because its separate discard-recycle attack choice is not part of this Ability slice', () => {
