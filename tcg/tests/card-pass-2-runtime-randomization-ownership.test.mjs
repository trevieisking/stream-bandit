...
test('private-alpha opening deck, mulligan and toss delegate randomness to the shared engine', () => {
  assert.ok(privateAlpha.includes('import { runtimeV02FlipCoin, runtimeV02ShuffledCopy } from "../_shared/tcg-match-randomization-engine-v0-2.ts";'));
  ...
  assert.ok(privateAlpha.includes('runtimeV02ApplyOpeningCoinCall(state,seat as 1|2,coinCall,runtimeV02FlipCoin)'));
  assert.equal(privateAlpha.includes('const toss=runtimeV02UniformRandomInt(2)+1'), false, 'opening toss must not preselect a winning seat before the player calls');
