/* Stream Bandit V6.72.1 Menu Saves Count Add-on
   V7.12.275 owner visibility + direct URL lock.
   Owner-only menu/header links stay hidden unless the live profile is platform owner.
   Owner-only direct URLs show a locked page for non-owner users.
   Keeps save counts working without touching the passed header/footer/theme shells. */
(function(){
'use strict';
const VERSION='V6.72.1 Menu Saves Count / V7.12.275 Owner Direct Lock Fix';
...
let sb=null,user=null,profile=null,isOwner=false,ownerChecked=false,ownerLocked=false,lastCounts={watchlist:0,favourites:0,likes:0,liked:0};
...
function applyOwnerVisibility(){ensureStyle();document.documentElement.classList.toggle('sb-owner-menu-visible',!!isOwner);document.documentElement.dataset.sbOwnerMenuVisibility=isOwner?'owner-visible':'owner-hidden';document.querySelectorAll('a[href]').forEach(a=>{if(isOwnerHref(a.getAttribute('href')))a.style.display=isOwner?'':'none'});document.querySelectorAll('#sbShellDrawer .sb-shell-group,#sbMenuDrawer .sb-menu-group').forEach(section=>{if(ownerSection(section))section.style.display=isOwner?'':'none'});if(isOwnerRoute()&&ownerChecked&&!isOwner)renderOwnerLock();if(isOwnerRoute()&&ownerChecked&&isOwner)document.documentElement.classList.remove('sb-owner-direct-pending')}
