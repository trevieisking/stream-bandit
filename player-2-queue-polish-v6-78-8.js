/* Stream Bandit V6.78.8 Player 2 Queue Polish. Multi-video queue player only. */
(function(){
  'use strict';
  var PLAYER_ROUTE = 'player-2-queue-watch-shell-v6-78-8-test.html';
  ...
  function activateQueueTab(){
    ...
    if(status) status.textContent = 'Player 2 active: Queue / Up Next. Multi-video mode.';
  }
  function navButton(label, item, index, q, cls){
    if(!item) return '<button class="btn '+(cls||'')+'" disabled>'+esc(label)+'</button>';
    return '<a class="btn '+(cls||'')+'" href="'+esc(itemUrl(item,index,q))+'">'+esc(label)+'</a>';
  }
  function chips(item, q){
    var gs = Array.isArray(item && item.genres) ? item.genres : [];
    if(!gs.length && q && q.genre) gs = [q.genre];
    return gs.slice(0,6).map(function(g){return '<span class="pill">'+esc(g)+'</span>';}).join('');
  }
  function renderQueue(){
    ...
    if(!q || !isQueueRequest(q)){
      box.innerHTML = '<h2>Player 2 — Queue Mode</h2><div class="note">No queue is loaded. Player 2 only appears when a real Play All queue exists.</div>';
      return;
    }
    var items = q.items || [];
    ...
    html += '</div>';
    html += '<div class="actions" style="margin:16px 0 4px">';
    html += navButton('⬅ Previous', previousItem, currentIndex - 1, q, '');
    html += navButton('▶ Next', nextItem, currentIndex + 1, q, 'hot');
    html += '<a class="btn primary" href="'+DETAILS_ROUTE+'?id='+encodeURIComponent(String(current.id || ''))+'">Current Details</a>';
    html += '</div>';
    if(upNext.length){
      html += '<h2 style="margin-top:22px">Up Next</h2><div class="checks">';
      upNext.forEach(function(item, offset){
        var index = currentIndex + offset + 1;
        html += '<div class="check"><b>'+esc(item.title || ('Movie '+(index+1)))+'</b>';
        html += '<p>Queue position '+(index+1)+' of '+items.length+'</p><div>'+chips(item,q)+'</div>';
      ...
      html += '<div class="note">This is the final movie in the queue.</div>';
    }
    if(previous.length){
      html += '<h2 style="margin-top:22px">Previous</h2><div class="checks">';
      previous.slice(-6).forEach(function(item){
        var index = items.findIndex(function(x){return String(x.id) === String(item.id);});
        html += '<div class="check"><b>'+esc(item.title || 'Previous movie')+'</b><p>Already played in this queue.</p><div class="actions"><a class="btn small" href="'+esc(itemUrl(item,index,q))+'">Replay</a></div></div>';
      });
      html += '</div>';
    }
    box.innerHTML = html;
  ...
  function polishBase(){
    document.title = 'Stream Bandit V6.78.8 Player 2 Queue Polish TEST';
    var badge = document.querySelector('.badge');
    if(badge) badge.textContent = 'V6.78.8 Player 2 Queue Polish TEST';
    var heroTitle = document.getElementById('title');
    if(heroTitle && isQueueRequest(readQueue())) heroTitle.textContent = '▶️ Player 2 — Queue Mode';
    var hero = document.querySelector('.hero p');
    if(hero) hero.textContent = 'Player 2 is the multi-video player for Play All queues. Single videos stay on Player 1.';
    var footer = document.querySelector('.footer');
    if(footer) footer.textContent = 'V6.78.8 Player 2 Queue Polish TEST — Player 2 queue mode.';
    var checklist = document.querySelector('#checklist h2');
    if(checklist) checklist.textContent = 'V6.78.8 Player 2 Queue Checklist';
    var queueTab = document.querySelector('[data-tab="queue"]');
    if(queueTab) queueTab.textContent = 'Player 2 Queue';
  }
  function init(){
    polishBase();
    renderQueue();
    ...
    setTimeout(function(){polishBase();renderQueue();activateQueueTab();}, 600);
    setTimeout(function(){polishBase();renderQueue();activateQueueTab();}, 1500);
  }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', init); else init();
  window.StreamBanditPlayer2QueuePolishV6788 = { refresh: renderQueue, readQueue: readQueue, activateQueueTab: activateQueueTab };
})();
