/* Stream Bandit Social Group/Event RPC Remove V7.13.001
...
(function(){
'use strict';
const VERSION='V7.13.001 Group Event RPC Remove Helper';
...
function bind(){if(!active())return;document.addEventListener('click',function(e){let g=e.target&&e.target.closest&&e.target.closest('[data-remove-group]');if(g){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();removeGroup(g.getAttribute('data-remove-group'));return;}let ev=e.target&&e.target.closest&&e.target.closest('[data-remove-event]');if(ev){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();removeEvent(ev.getAttribute('data-remove-event'));return;}},true);window.StreamBanditSocialGroupEventRpcRemove={version:VERSION,removeGroup,removeEvent};document.documentElement.dataset.sbGroupEventRpcRemove='v7-13-001';}
