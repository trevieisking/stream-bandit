...
function loadClarity(){loadOne('assets/code-labs-setup-route-v145.js?v=cl-v145','data-cl-setup-route-v145');loadOne('assets/code-labs-workflow-clarity-v130.js?v=cl-workflow-clarity-v130','data-cl-workflow-clarity-v130');loadOne('assets/code-labs-save-language-v132.js?v=cl-save-language-v132','data-cl-save-language-v132');loadOne('assets/code-labs-workflow-guard-v138.js?v=cl-v138','data-cl-workflow-guard-v138');loadOne('assets/code-labs-page-completion-v139.js?v=cl-v139','data-cl-page-completion-v139')}
...
function restoreTitleTop(){var main=q('.main');if(!main)return;var hero=q('.hero',main);if(!hero)return;var top=q('.topbar',main);if(top&&top.parentNode===main&&top.nextSibling!==hero){main.insertBefore(hero,top.nextSibling);return}if(!top&&main.firstChild!==hero){main.insertBefore(hero,main.firstChild)}}
function add(){loadClarity();restoreTitleTop();var main=q('.main');if(!main||q('#clBackupPanel'))return;var p=document.createElement('section');p.className='panel';p.id='clBackupPanel';p.innerHTML='<h2>Backup tools</h2><p>Before promotion, keep a copy of this browser state and a short summary.</p><div class="actions"><button class="btn primary" id="clExportState">Export state</button><button class="btn ghost" id="clCopyStateSummary">Copy summary</button></div>';var f=q('.footerNote');if(f&&f.parentNode)f.parentNode.insertBefore(p,f);else main.appendChild(p);q('#clExportState').onclick=function(){saveFile('code-labs-state.json',JSON.stringify(read(),null,2))};q('#clCopyStateSummary').onclick=function(){copy(summary())}}
function boot(){add();setTimeout(restoreTitleTop,450);setTimeout(restoreTitleTop,1100);setTimeout(restoreTitleTop,2300);setTimeout(restoreTitleTop,4200)}
