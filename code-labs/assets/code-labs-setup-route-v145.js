/* Code Labs Setup Route V228 - keeps Setup after Home and routes Setup content directly to File Lab. No writes. */
(function(){
'use strict';
...
function rewriteProjectPickerLinks(){qa('a[href="project-picker.html"]').forEach(function(a){if(a.closest&&a.closest('.nav'))return;a.href='file-lab.html';if(/next/i.test(a.textContent||''))a.textContent='Next: File Lab';else if(/project/i.test(a.textContent||''))a.textContent='File Lab'})}
function fixRoutePanels(){var id=(document.body&&document.body.getAttribute('data-page'))||'';var box=q('#clWorkflowClarityV130'),next=box&&q('.next',box);if(id==='index'&&next){next.href='setup.html';next.textContent='Next: Setup'}if(id==='setup'&&next){next.href='file-lab.html';next.textContent='Next: File Lab'}if(id==='setup')rewriteProjectPickerLinks();var foot=q('#clHelpShortcut');if(foot&&/Home\s*→\s*File Lab/.test(foot.textContent||'')){foot.innerHTML=foot.innerHTML.replace('Home → File Lab','Home → Setup → File Lab')}}
...
function run(){keepSetup();fixRoutePanels();watch();document.body.setAttribute('data-code-labs-setup-route-v145','active');window.CodeLabsSetupRouteV145={version:'V228',active:true,route:'Home -> Setup -> File Lab'}}
