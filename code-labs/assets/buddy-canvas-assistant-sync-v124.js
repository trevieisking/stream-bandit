...
(function(){
  ...
  async function updateTree(sb,tree,i,sig,reason,now){
    ...
    if(tree.file_id){var fr=await cu.sb.from('code_labs_files').update(filePayload).eq('id',tree.file_id).select('id').maybeSingle();if(fr.error)throw fr.error;if(!fr.data)throw new Error('Assistant current-file row was not available.');}
