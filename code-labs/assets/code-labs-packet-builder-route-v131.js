...
function addPacketMenu(id){var nav=q('.nav');if(!nav)return;var existing=q('.nav a[href="packet-builder.html"]');if(!existing){var a=document.createElement('a');a.href='packet-builder.html';a.innerHTML='<span>Packet</span><div>Packet Builder<small>Build packet</small></div>';var rescue=q('.nav a[href="rescue-room.html"]');if(rescue&&rescue.nextSibling)nav.insertBefore(a,rescue.nextSibling);else nav.appendChild(a);existing=a}if(id==='packet-builder'){Array.prototype.slice.call(nav.querySelectorAll('a')).forEach(function(x){x.classList.toggle('active',x.getAttribute('href')==='packet-builder.html')})}}
...
setTimeout(run,200);setTimeout(run,700);setTimeout(run,1500);setTimeout(run,3100);setTimeout(run,4300);
