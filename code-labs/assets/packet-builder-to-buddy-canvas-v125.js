...
(function(){
  ...
  function syncVisibleFields(target){
    ...
    f.githubSource.path=f.githubSource.path||f.path||filename;
    ...
    if(!p.siteUrl)p.siteUrl=filename;
    ...
    s.buddyCanvasTarget={
      ...
      filename:filename,
      path:f.githubSource.path||filename,
    ...
    s.log.unshift({id:'cl_'+Date.now(),date:new Date().toLocaleString(),msg:'Packet Builder sent '+filename+' to Buddy Canvas target '+(target||'chatgpt')});
  ...
  function send(target){
    ...
    toast('Loaded for Buddy Canvas: '+((s.file&&s.file.filename)||'file'));
    var url='buddy-canvas.html?source=packet-builder&target='+encodeURIComponent(target||'chatgpt')+'&file='+encodeURIComponent((s.file&&s.file.filename)||'file.html')+'&v=125';
  ...
  function boot(){addButtons();setTimeout(addButtons,450);setTimeout(addButtons,1200);window.CodeLabsPacketToBuddyV125={send:send,syncVisibleFields:syncVisibleFields};}
