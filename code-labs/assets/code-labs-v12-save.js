...
(function(){
  ...
  function navLink(href,icon,title,small){
    var a=document.createElement('a');
    a.href=href;
    if(location.pathname.indexOf('/'+href)!==-1)a.className='active';
    a.innerHTML='<span>'+icon+'</span><div>'+title+'<small>'+small+'</small></div>';
    return a;
  }
  function addExtraMenus(){
    ...
    if(!nav)return;
    var items=[
      ['start-guide.html','🟢','Start Guide','Tell us what broke','index.html'],
      ['fix-wizard.html','🧙','Fix Wizard','One next step','start-guide.html'],
      ['v20.html','🧰','Workflow Hub','One place helper','fix-wizard.html'],
      ['patch-lab.html','🧠','Patch Lab','Find and replace safely','patch-desk.html'],
      ['ai-handoff.html','🤖','AI Handoff','Send fix back to ChatGPT','packet-builder.html'],
      ['publish-prep.html','🚦','Publish Prep','Safe test branch request','ai-handoff.html'],
      ['repo-desk.html','🗄️','Repo Desk','Plan file work','publish-prep.html'],
      ['github-tracker.html','🧭','GitHub Tracker','Track test links','repo-desk.html']
    ];
    items.forEach(function(i){
      if(document.querySelector('a[href="'+i[0]+'"]'))return;
      var a=navLink(i[0],i[1],i[2],i[3]);
      var ref=nav.querySelector('a[href="'+i[4]+'"]');
  ...
  function insertAfterFirstPanel(panel){
  ...
  function addRepairLivePanel(){
    var p=page();
    if(REPAIR_PAGES.indexOf(p)===-1)return;
    if(document.querySelector('#clRepairLivePanel'))return;
    var labels={
      'rescue-room':['Rescue Room','save the problem, rules, and error notes before building a packet'],
      'packet-builder':['Packet Builder','build a complete ChatGPT help packet from the saved repair state'],
      'patch-desk':['Patch Desk','paste the fixed full file, compare it, checkpoint it, and move to preview']
    };
    var info=labels[p]||['Repair page','continue the repair flow'];
    var panel=document.createElement('section');
    panel.className='panel';
    panel.id='clRepairLivePanel';
  ...
  function addHelpOperatingMap(){
    ...
    var first=document.querySelector('.panel');
    if(first&&first.parentNode){first.parentNode.insertBefore(panel,first);}else{insertAfterFirstPanel(panel);}
  }
  ...
  function polishStatus(){
    if(page()!=='connector-status')return;
