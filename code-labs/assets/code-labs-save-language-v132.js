...
function meaningPanel(){var id=page(),msg=MEANING[id];if(!msg||q('#clSaveMeaningV132'))return;var target=q('#clWorkflowClarityV130')||q('#clNextFlowPanel')||q('.topbar');var box=document.createElement('div');box.id='clSaveMeaningV132';box.className='clSaveLanguageV132';box.innerHTML='<b>Save meaning on this page</b><small>'+esc(msg)+'</small>';if(target&&target.parentNode)target.parentNode.insertBefore(box,target.nextSibling);else{var main=q('.main')||document.body;main.insertBefore(box,main.firstChild)}}
