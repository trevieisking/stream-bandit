...
function genericButton(){var id=page();if(q('#clGenericNextStep'))return;var map={
...
var m=map[id];if(!m)return;var ready=id==='github-tracker'||hasFixed();var wrap=document.createElement('section');wrap.id='clGenericNextStep';wrap.className='panel clNextStepWrap '+(ready?'':'bad');wrap.innerHTML='<h2>Big next button</h2><p><b>'+m.note+'</b></p>'+(ready?'<p>Safe next click is below.</p>':'<p>Fixed output is not saved yet. Go back to Patch Lab first if this is a repair.</p>');wrap.appendChild(makeButton(m.href,m.text,ready));var main=q('.main');var after=q('#buddyLaneGuide')||q('#clNextFlowPanel')||q('.topbar');if(after&&after.parentNode)after.parentNode.insertBefore(wrap,after.nextSibling);else if(main)main.insertBefore(wrap,main.firstChild)}
