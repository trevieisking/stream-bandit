/* Code Labs Start Family V251 - Home, Setup and Project Picker presentation only. */
(function(){
'use strict';
var PAGE=(document.body&&document.body.getAttribute('data-page'))||'';
if(['index','setup','project-picker'].indexOf(PAGE)<0)return;
var observer=null,stopTimer=0;
function q(s,r){return(r||document).querySelector(s)}
function all(s,r){return Array.prototype.slice.call((r||document).querySelectorAll(s))}
function add(n,c){if(n)n.classList.add(c)}
function restoreNav(){var nav=q('.nav');if(!nav||!window.CodeLabsShellLoaderV235||!window.CodeLabsShellLoaderV235.firstNav)return;var links=all('a[href]',nav);if(links.length!==18||nav.getAttribute('data-cl-nav-owner')!=='V251-first-paint')window.CodeLabsShellLoaderV235.firstNav()}
...
function apply(){var main=q('.main')||q('main');if(!main)return false;restoreNav();document.body.classList.add('clStartFamilyV251');style();if(PAGE==='index')home(main);if(PAGE==='setup')setup(main);if(PAGE==='project-picker')picker(main);document.body.setAttribute('data-cl-start-family','v251');return true}
function boot(){apply();var root=document.body;if(root){observer=new MutationObserver(apply);observer.observe(root,{childList:true,subtree:true})}stopTimer=setTimeout(function(){if(observer)observer.disconnect();apply()},6000)}
window.CodeLabsStartFamilyV251={version:'V251',apply:apply,page:PAGE};
