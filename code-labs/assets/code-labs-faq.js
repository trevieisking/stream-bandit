/* Code Labs FAQ V3.6 - current public FAQ */
(function(){
  'use strict';
  ...
  function render(){
    ...
    main.innerHTML=(top?top.outerHTML:'')+
      '<section class="hero" id="clFaqPage"><div><span class="pill">Code Labs FAQ</span><h1>Clear answers for the current Code Labs workflow.</h1><p>This FAQ explains what Code Labs is for, how to use it, what each page does, what is stored locally, what uses Supabase Repair History, and how the GitHub handoff pages fit into the repair lane.</p><div class="actions"><a class="btn primary" href="file-lab.html">Start File Lab</a><a class="btn good" href="patch-lab.html">Open Patch Lab</a><a class="btn ghost" href="about.html">About Code Labs</a></div></div><div class="heroCard"><b>Normal safe flow</b><ol><li>File Lab: load or paste full code.</li><li>Rescue Room: describe the problem.</li><li>Workflow Hub: choose the request route.</li><li>Patch Desk or Patch Lab: save fixed output.</li><li>Preview + Test: check before live.</li><li>Checkpoints: keep rollback.</li><li>GitHub Writer and Tracker: record branch/PR follow-up.</li></ol></div></section>'+ 
      '<section class="panel"><h2>What Code Labs is</h2><div class="grid3">'+
      item('What is Code Labs?','Code Labs is a ChatGPT repair room for website owners and non-coders. It helps collect full-file context, problem notes, protected rules, fixed output, preview tests, checkpoints, repository follow-up, and repair history.', ['Repair room','good'])+
      item('Who is Code Labs for?','It is for people maintaining website files who need ChatGPT help but do not want to lose working login, saves, menus, media, database behaviour, routes, or accessibility features.', ['Non-coder friendly','good'])+
      ...
      item('What does Home do?','Home explains the current Code Labs purpose, shows the repair state from browser storage, and gives the main next clicks.')+
      ...
      item('What does Workflow Hub do?','Workflow Hub chooses the route: read request, repair request, review, exact patch, GitHub help, or Supabase help.')+
      ...
      item('What is Supabase Repair History?','It is the footer panel that can save Code Labs projects, files, jobs, versions, packets, tests, and audit notes to dedicated Code Labs tables when Supabase is connected.')+
      item('Can I clear old Supabase history?','Yes. Loaded history entries can show a Remove button. It clears the selected Code Labs history entry only; it is not for website pages or repository files.', ['Current feature','good'])+
      ...
      item('What should I test before promotion?','Check that the page opens, buttons work, mobile layout is acceptable, no obvious error text appears, login/saves still work where relevant, and a checkpoint exists.')+
      item('Why does Code Labs ask for a full file?','Full files reduce guessing. A tiny snippet can miss working logic, scripts, styles, settings, routes, or database calls that must be preserved.')+
      '</div></section>'+ 
      ...
      item('When do I use Supabase help?','Use Supabase help for Code Labs Repair History, table checks, data policy questions, or database planning. Keep it separate from GitHub file tasks.')+
      item('Why one connector lane at a time?','One lane at a time keeps the job clear. GitHub is for repo files and PR tracking. Supabase is for Code Labs data and history.')+
      ...
      '<div class="footerNote">Code Labs FAQ V3.6 - current public guide for the live repair workflow.</div>';
