/* Code Labs Footer and V104 Relay V202.3
   One self-healing relay instance, one interval, one listener set. No Code Labs login page.
*/
(function(){
'use strict';
...
var VERSION='V202.3-self-healing-v104-relay';
var ROUTES=[
...
var busy=false,timer=0,started=false,listeners=false,lastError='';
...
function remember(d){try{sessionStorage.setItem(SESSION,d.session_id||'');sessionStorage.setItem(SECRET,d.browser_secret||stored(SECRET));lastError=''}catch(e){}}
...
async function tick(){if(busy||document.hidden)return;var packet=snapshot();if(!packet)return;busy=true;try{if(!stored(SESSION)||!stored(SECRET)){remember(await invoke(body('create_pairing',packet)));return}await invoke(body('heartbeat',packet));var data=await invoke(body('poll',null));if(data.command){var receipt=await apply(data.command),out=body('receipt',snapshot()||packet);out.command_id=data.command.id;out.receipt=receipt;await invoke(out)}lastError=''}catch(e){lastError=String(e.message||e);clear()}finally{busy=false}}
function start(){if(started)return;started=true;tick();timer=setInterval(tick,2200);if(!listeners){listeners=true;document.addEventListener('visibilitychange',tick);window.addEventListener('pageshow',tick)}window.CodeLabsV104PageRelay={version:VERSION,run:tick,clear:clear,status:function(){return{started:started,session:!!stored(SESSION),page:page(),last_error:lastError}}}}
function routeIndex(){var id=page();for(var i=0;i<ROUTES.length;i++)if(ROUTES[i][0]===id)return i;return-1}
function addFooter(){var main=q('.main')||q('main');if(!main||q('#clFooterBuddyShellV201'))return;var i=routeIndex();if(i<0)return;var prev=i>0?ROUTES[i-1]:null,next=i<ROUTES.length-1?ROUTES[i+1]:null,f=document.createElement('section');f.id='clFooterBuddyShellV201';f.className='panel';f.innerHTML='<h2>Safe next step</h2><p>V104 can read this visible page, write approved fields, press approved buttons, return receipts and undo the latest page-field write.</p><div class="actions">'+(prev?'<a class="btn ghost" href="'+prev[1]+'">Previous: '+prev[2]+'</a>':'')+(next?'<a class="btn primary" href="'+next[1]+'">Next: '+next[2]+'</a>':'')+'<a class="btn ghost" href="help.html">Help + Tools</a></div><p class="fine">'+VERSION+' · one V104 connector · GitHub writes remain branch and pull request only.</p>';main.appendChild(f)}
function loadBridge(){if(bridge()||q('script[data-cl-buddy-page-bridge-v202]'))return;var s=document.createElement('script');s.src='assets/code-labs-buddy-page-bridge-v139.js?v=cl-v202';s.setAttribute('data-cl-buddy-page-bridge-v202','yes');s.onload=start;document.head.appendChild(s)}
function run(){addFooter();if(bridge())start();else loadBridge();return true}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run();
window.CodeLabsFooterBuddyShellV202={version:VERSION,routes:ROUTES,run:run};window.CodeLabsFooterBuddyShellV201=window.CodeLabsFooterBuddyShellV202;window.CodeLabsFooterBuddyShellV200=window.CodeLabsFooterBuddyShellV202;
})();