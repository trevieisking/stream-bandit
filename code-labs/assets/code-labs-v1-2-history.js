/* Code Labs V1.2.4 - Supabase saved repair history + file-row rendering */
(function(){
  'use strict';
  ...
  function projectPayload(s){var p=s.project||{};return {workspace:p.workspace||'',site_name:p.siteName||'Untitled project',site_url:p.siteUrl||'',repo:p.repo||'',mode:p.mode||'manual',notes:p.notes||'',metadata:{source:'code-labs-v1.2.4',domain:location.hostname}};}
  ...
  async function saveAll(){
    try{
      ...
      var fr=await cu.sb.from('code_labs_files').insert({project_id:projectId,filename:f.filename||'file.html',file_type:(f.filename||'').split('.').pop()||'html',current_code:f.currentCode||'',current_hash:String((f.currentCode||'').length),metadata:{site:p.siteName||'',source:'code-labs-v1.2.4'}}).select('id').single();
      ...
      var versions=[];
      if(f.currentCode)versions.push({project_id:projectId,job_id:jobId,file_id:fileId,version_kind:'original',label:'Original code',filename:f.filename||'file.html',code:f.currentCode,note:'Saved from Code Labs V1.2.4'});
      if(f.fixedCode)versions.push({project_id:projectId,job_id:jobId,file_id:fileId,version_kind:'fixed',label:'Fixed code',filename:f.filename||'file.html',code:f.fixedCode,note:'Saved from Code Labs V1.2.4'});
      ...
      await cu.sb.from('code_labs_audit_log').insert({project_id:projectId,job_id:jobId,action:'saved_repair_history',details:{filename:f.filename||'',hasFixed:!!f.fixedCode,tests:tests.length,source:'code-labs-v1.2.4'}});
