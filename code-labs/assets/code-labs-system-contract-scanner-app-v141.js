(() => {
  ...
  function render(report) {
    const counts=[['Files scanned',report.files_scanned],['Pages',report.pages.length],['Dependencies',report.dependencies.length],['Environment calls',report.environment_variables.length],['DB/RPC entries',report.database_rpc.length],['Tool routes',report.tool_routing.length],['Blockers',report.findings.blockers.length],['Warnings',report.findings.warnings.length]];
    C.$('summary').innerHTML=counts.map(([label,value])=>`<div class="card"><strong>${C.esc(value)}</strong>${C.esc(label)}</div>`).join('');
    C.$('findings').innerHTML=`<h3 class="${report.findings.ready?'good':'bad'}">${C.esc(report.findings.headline)}</h3><details open><summary>Blockers (${report.findings.blockers.length})</summary>${report.findings.blockers.map(x=>`<div class="bad">${C.esc(x)}</div>`).join('')||'<div class="good">None found by static rules.</div>'}</details><details><summary>Warnings (${report.findings.warnings.length})</summary>${report.findings.warnings.map(x=>`<div class="warn">${C.esc(x)}</div>`).join('')}</details>`;
    ...
    C.$('environment').innerHTML=C.table(['Variable','Call','File','Line','Secret-like','Browser exposure'],report.environment_variables.map(x=>[x.name,x.call,x.file,x.line,x.secret_like,x.browser_exposure]));
    C.$('database').innerHTML=C.table(['Target','Operation','Channel','File','Line','Evidence'],report.database_rpc.map(x=>[x.target,x.operation,x.channel,x.file,x.line,x.evidence]));
    C.$('routing').innerHTML=C.table(['Tool','Annotation','Handler','Module','File','Line','Status'],report.tool_routing.map(x=>[x.tool,x.annotation,x.handler,x.module,x.file,x.line,x.status]));
    C.$('authentication').innerHTML=C.table(['Page','data-page','Classification','Evidence','Live HTTP'],report.pages.map(x=>[x.path,x.data_page,x.auth.classification,x.auth.evidence,x.live_http?`${x.live_http.status} ${x.live_http.ok?'OK':'FAIL'}`:'not imported']));
    C.$('workflow').innerHTML=C.table(['Page','Step','Expected next','Links','Storage reads','Storage writes','Direct mutations','Warnings'],report.cross_page_workflow.map(x=>[x.page,x.step,x.expected_next,x.links,x.storage_reads,x.storage_writes,x.direct_browser_mutations,x.warnings]));
  ...
  async function run() {
    ...
    try {
      ...
      for(const file of files) for(const reference of C.references(file.text)) { if(!reverseReferences.has(reference))reverseReferences.set(reference,[]); reverseReferences.get(reference).push(file.path); }
      const dependencies=files.filter(x=>x.path.startsWith('code-labs/')).map(file=>({path:file.path,type:/\.html?$/.test(file.path)?'page':/\.js$/.test(file.path)?'helper':/\.css$/.test(file.path)?'style':'asset',references:C.references(file.text),loaded_by:C.uniq(reverseReferences.get(file.path)||[]),storage_writes:C.storage(file.text,'setItem')}));
      ...
      C.setStatus(`Scan complete at exact commit ${resolved.commitSha}. ${report.findings.blockers.length} blocker(s), ${report.findings.warnings.length} warning(s).`,report.findings.blockers.length?'bad':'good');
