...
function stabilizeNav(){var nav=q('.nav');if(!nav)return false;if(nav.getAttribute('data-cl-nav-stable')===VERSION&&sameOrder(nav))return true;repairingNav=true;nav.innerHTML='';LINKS.forEach(function(item,i){nav.appendChild(makeLink(item,i))});nav.setAttribute('data-cl-nav-stable',VERSION);nav.setAttribute('aria-label','Code Labs simple workflow navigation');repairingNav=false;return true}
function watchNav(){var nav=q('.nav');if(!nav||nav.getAttribute('data-cl-nav-watch')===VERSION)return;nav.setAttribute('data-cl-nav-watch',VERSION);navObserver=new MutationObserver(function(){if(repairingNav)return;if(!sameOrder(nav)||nav.getAttribute('data-cl-nav-stable')!==VERSION)stabilizeNav()});navObserver.observe(nav,{childList:true,subtree:true,attributes:true,attributeFilter:['href','data-cl-nav-stable']})}
...
function run(){var ok=stabilizeNav();if(!ok){setTimeout(run,100);return}watchNav();addBuddyBox();setupNextToSavedFiles();loadWorkflowClarity();loadRouteAgreement();loadPagePolish();loadPatchDeskDedupe();loadOneFlow();loadSolWorkbench()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run);else run();setTimeout(run,250);setTimeout(run,900);setTimeout(run,3200);
