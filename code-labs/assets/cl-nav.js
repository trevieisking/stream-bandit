(function(){
'use strict';
var VERSION='v179-v145-every-page-write-entry';
var LINKS=[
...
function loadScriptOnce(src,attr,onload){var old=q('script['+attr+']')||q('script[src*="'+src.split('?')[0]+'"]');if(old){if(onload)onload();return old}var s=document.createElement('script');s.src=src;s.setAttribute(attr,'yes');if(onload)s.onload=onload;document.head.appendChild(s);return s}
function loadV145Runtime(){loadScriptOnce('assets/code-labs-page-runtime-v145.js?v=cl-v145-every-page-write','data-cl-page-runtime-v145')}
...
function run(){var ok=stabilizeNav();if(!ok){setTimeout(run,100);return}loadV145Runtime();addBuddyBox();setupNextToFileLab();loadWorkflowClarity();loadRouteAgreement();loadBuddyLaneGuide();loadNextButtons();loadCurrentFilePanel();loadPagePolish();loadPatchDeskDedupe();loadSolWorkbench()}
