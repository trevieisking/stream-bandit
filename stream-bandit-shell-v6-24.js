/* Stream Bandit V6.24 Legacy Shell Bridge
   Retired visual shell. Filename kept because many old pages still load it and read Supabase config from it.
   This file must NOT inject the old header, old footer, floating menu, or old account overlay.
   It only exposes config/routes and loads the approved V7.12.156 foundation. */
(function(){
'use strict';
const VERSION='V6.24 Legacy Shell Bridge -> V7.12.156 Foundation';
const DEF='test-page';
...
const FOUNDATION='v7-12-156-foundation-bridge';
const R={
...
const FIX={
  ...
  'brand-logo-helper-responsive-v7-12-20-test.html':R.brandIcons,
  'brand-image-helper-v7-12-20-test.html':R.brandIcons,
  'favicon-app-icon-builder-v7-12-15-test.html':R.brandIcons
};
...
function pageRoute(route,oldValue){let out=String(route||'');if(out.includes('page=test-page')){let p=pageParam();try{let u=new URL(String(oldValue||''),location.href);p=u.searchParams.get('page')||p;}catch(e){}out=out.replace('page=test-page','page='+encodeURIComponent(p));}return out;}
function fixedUrl(raw){if(!raw||!String(raw).includes('.html'))return raw;let target=FIX[file(raw)];return target?pageRoute(target,raw):raw;}
function loadScript(src){try{let base=src.split('?')[0];if(Array.from(document.scripts||[]).some(s=>String(s.src||'').includes(base)))return;let s=document.createElement('script');s.src=src;s.defer=true;s.dataset.sbLoadedBy='legacy-shell-bridge';document.head.appendChild(s);}catch(e){}}
function setFavicon(){try{let existing=document.querySelector('link[rel="icon"]')||document.createElement('link');existing.rel='icon';existing.href=FAVICON_URL;document.head.appendChild(existing);}catch(e){}}
function hideLegacyShellArtifacts(){try{document.querySelectorAll('.sb-shell-menu-toggle,.sb-shell-scrim,.sb-shell-drawer,.sb104-right,.sb104-icons,.sb104-account,.sb104-profile,.sb104-search,.sb104-panel,#sbSharedShellMenu,#sbSharedShellDrawer,#sbSharedShellScrim').forEach(el=>{el.remove();});}catch(e){}try{document.querySelectorAll('.head.sb104-head').forEach(el=>el.classList.remove('sb104-head'));}catch(e){}try{document.querySelectorAll('style#sbSharedShellStyle').forEach(el=>el.remove());}catch(e){}document.documentElement.dataset.sbLegacyVisualShellRetired='true';}
function patchRoutes(root){let changed=0;root=root||document;try{root.querySelectorAll('a[href],form[action],[data-href],[data-target],[data-route],[data-url]').forEach(el=>{['href','action','data-href','data-target','data-route','data-url'].forEach(attr=>{let old=el.getAttribute(attr),fix=fixedUrl(old);if(old&&fix&&fix!==old){el.setAttribute(attr,fix);el.dataset.sbRouteFixedBy='legacy-shell-bridge';changed++;}});});}catch(e){}return changed;}
function expose(){window.StreamBanditRoutes=Object.assign(window.StreamBanditRoutes||{},R);window.StreamBanditSupabaseConfig={url:SUPABASE_URL,key:SUPABASE_KEY,source:'stream-bandit-shell-v6-24.js'};window.StreamBanditShellConfig=window.StreamBanditSupabaseConfig;window.SUPABASE_URL=window.SUPABASE_URL||SUPABASE_URL;window.SUPABASE_KEY=window.SUPABASE_KEY||SUPABASE_KEY;}
function ensureFoundation(){loadScript('stream-bandit-theme-projector-v7-12-156.js?v='+FOUNDATION);loadScript('stream-bandit-header-shell-v7-12-156.js?v='+FOUNDATION);loadScript('stream-bandit-footer-shell-v7-12-156.js?v='+FOUNDATION);loadScript('stream-bandit-global-helper-loader-v7-12-126.js?v='+FOUNDATION);loadScript('live-readiness-search-supabase-fallback-v7-12-130.js?v='+FOUNDATION);loadScript('stream-bandit-core-saves-v6-75.js?v='+FOUNDATION);loadScript('stream-bandit-movie-card-saves-v7-12-156.js?v='+FOUNDATION);}
function refresh(){expose();setFavicon();hideLegacyShellArtifacts();patchRoutes(document);ensureFoundation();try{if(window.StreamBanditThemeProjector&&window.StreamBanditThemeProjector.refresh)window.StreamBanditThemeProjector.refresh();}catch(e){}try{if(window.StreamBanditHeaderShell&&window.StreamBanditHeaderShell.refresh)window.StreamBanditHeaderShell.refresh();}catch(e){}try{if(window.StreamBanditFooterShell&&window.StreamBanditFooterShell.refresh)window.StreamBanditFooterShell.refresh();}catch(e){}try{if(window.StreamBanditMovieCardSaves&&window.StreamBanditMovieCardSaves.refresh)window.StreamBanditMovieCardSaves.refresh();}catch(e){}return state();}
function state(){return{version:VERSION,visualShellRetired:true,foundation:true,routes:R,supabase:{url:SUPABASE_URL,keyPresent:!!SUPABASE_KEY}};}
function boot(){refresh();setTimeout(refresh,200);setTimeout(refresh,800);setTimeout(refresh,1800);setInterval(function(){hideLegacyShellArtifacts();patchRoutes(document);ensureFoundation();},2500);try{let obs=new MutationObserver(function(){hideLegacyShellArtifacts();patchRoutes(document);});obs.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['href','action','data-href','data-target','data-route','data-url']});}catch(e){}window.StreamBanditShell={version:VERSION,routes:R,refresh,state,config:()=>({url:SUPABASE_URL,key:SUPABASE_KEY})};window.StreamBanditLegacyShellBridge=window.StreamBanditShell;document.documentElement.dataset.sbLegacyShellBridge='v7-12-156';}
