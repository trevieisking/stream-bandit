/* Stream Bandit V7.12.40 Shared Shell Clean Machine Menu
...
const VERSION='V7.12.40 Shared Shell Clean Machine Menu';
...
function applyRouteGuard(root=document){try{root.querySelectorAll&&root.querySelectorAll('a[href],form[action],[data-href],[data-target],[data-route],[data-url]').forEach(el=>{['href','action','data-href','data-target','data-route','data-url'].forEach(attr=>{const val=el.getAttribute&&el.getAttribute(attr);if(!val)return;const fixed=fixedRouteValue(val);if(fixed!==val){el.setAttribute(attr,fixed);el.dataset.sbRouteGuard='fixed';}});});document.documentElement.dataset.streamBanditRouteGuard='v7-12-40';}catch(e){}}
