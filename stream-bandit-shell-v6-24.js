/* Stream Bandit V7.6.3 Shared Shell / Menu Control Tower Label + V7.12.16 Global Favicon
   Safe UI pass only: keeps grouped drawer/search behaviour and labels Platform Builder as Platform Control Tower.
   Adds global browser tab/favicon from Stream Bandit brand logo.
   No saves, uploads, deletes, billing, permission writes, database writes or live/index promotion. */
(function(){
'use strict';

const VERSION='V7.6.3 Shared Shell Menu Control Tower Label + V7.12.16 Global Favicon';
...
const GLOBAL_FAVICON_PARTS=[
  'https://xzxqfrvqdgkzwujbkdbk.supabase',
  '.co/storage/v1/object/public/',
  'stream-bandit-images/global/',
...
function poster(m){
 return m.thumbnail_url||m.poster_url||m.image_url||m.backdrop_url||m.thumb||'';
}
...
function sbGlobalFaviconUrl(){
 return GLOBAL_FAVICON_PARTS.join('');
}

function sbSetFaviconLink(rel,sizes,url){
 let link =
  document.querySelector('link[rel="'+rel+'"][data-sb-global-favicon="true"]') ||
 ...
 link.dataset.sbGlobalFavicon='true';
 if(sizes)link.sizes=sizes;
 link.href=url;
 return link;
}

function applyStreamBanditGlobalFavicon(){
 try{
  const cleanUrl=sbGlobalFaviconUrl();
  const bustedUrl=cleanUrl+(cleanUrl.includes('?')?'&':'?')+'sbFav='+Date.now();

  sbSetFaviconLink('icon','32x32',bustedUrl);
  sbSetFaviconLink('shortcut icon','32x32',bustedUrl);
  sbSetFaviconLink('apple-touch-icon','180x180',bustedUrl);

  document.documentElement.dataset.streamBanditFavicon='loaded';

  document.dispatchEvent(new CustomEvent('streambandit:favicon-loaded',{
   detail:{
    version:'V7.12.16 Global Favicon via Shared Shell',
    url:cleanUrl,
    source:'stream-bandit-shell-v6-24.js'
   }
  }));
 }catch(e){
  // Favicon must never break the page.
 }
}
...
function injectDrawer(){
 ['menuToggle','drawer','scrim','sbShellMenuToggle','sbShellDrawer','sbShellScrim'].forEach(id=>{
  const el=document.getElementById(id);
  if(el)el.remove();
 });

 const btn=document.createElement('button');
 btn.id='sbShellMenuToggle';
 btn.className='sb-shell-menu-toggle';
 btn.textContent='☰';
 ...
 drawer.id='sbShellDrawer';
 drawer.className='sb-shell-drawer';
 ...
 btn.onclick=open;
 scrim.onclick=close;
 drawer.querySelector('#sbShellClose').onclick=close;

 drawer.querySelector('#sbShellFilter').oninput=function(){
  const q=this.value.toLowerCase();
  drawer.querySelectorAll('[data-sb-wrap]').forEach(w=>{
   let any=false;
   w.querySelectorAll('.sb-shell-link').forEach(a=>{
    const hit=!q||a.dataset.sbText.includes(q);
    a.classList.toggle('sb-shell-hidden',!hit);
    if(hit)any=true;
   });
   w.classList.toggle('sb-shell-hidden',!any);
   if(q&&any)w.classList.add('open');
  });
 };

 drawer.addEventListener('click',e=>{
  const g=e.target.closest('[data-sb-group]');
  if(g)g.closest('.sb-shell-group').classList.toggle('open');
 });
...
function ensureSearch(){
 ...
 let overlay=wrap.querySelector('.sb-search-overlay')||document.getElementById('searchOverlay');
...
function fullSearch(input){
 ...
 location.href=GLOBAL_SEARCH+(q?'?q='+encodeURIComponent(q):'');
}
...
function renderSearch(input,overlay){
 ...
 const title=overlay.querySelector('#searchTitle');
 ...
 movieHits.forEach(m=>{
  ...
  const d=snippet(m.description||'No description yet.');
  html+=`<a class="sb-search-result" href="${CURRENT_DETAILS}?id=${id}"><span class="sb-search-thumb">${im?`<img src="${esc(im)}" alt="">`:'🎬'}</span><span><b>${esc(m.title||'Untitled')}</b><small>Movie result — Details</small><span class="sb-search-desc">${esc(d)}</span></span></a>`;
 });

 pageHits.forEach(p=>{
  html+=`<a class="sb-search-result" href="${p[2]}"><span class="sb-search-thumb">${p[0]}</span><span><b>${esc(p[1])}</b><small>Page result</small><span class="sb-search-desc">${esc(p[3]||'')}</span></span></a>`;
 });
...
async function loadMovies(){
 try{
  ...
  const r=await c.from('sb_movies').select('*').order('created_at',{ascending:false}).limit(150);
  if(!r.error)movies=(r.data||[]).filter(m=>String(m.status||'published')!=='archived');
 }catch(e){}
}

async function init(){
 applyStreamBanditGlobalFavicon();
 addStyle();
 injectDrawer();
 await loadMovies();
 ...
 if(s){
  ...
  s.input.addEventListener('keydown',e=>{
   ...
   if(e.key==='Escape')s.overlay.classList.remove('open');
  },true);

  if(s.button){
   s.button.addEventListener('click',e=>{
    e.preventDefault();
    fullSearch(s.input);
   },true);
  }

  const close=s.overlay.querySelector('#closeSearch');
  if(close)close.onclick=()=>s.overlay.classList.remove('open');

  document.addEventListener('click',e=>{
   if(!s.wrap.contains(e.target))s.overlay.classList.remove('open');
  });
 }

 document.documentElement.dataset.streamBanditShell='v7-6-3-global-favicon';
}
