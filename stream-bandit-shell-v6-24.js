/* Stream Bandit V6.24 Legacy Shell Bridge
   ...
   V7.12.176 corrects Group Play route truth:
   Collections -> collections-clean-machine-v7-12-51-test.html
   Player 2 -> player-2-clean-machine-v7-12-58-test.html
   No visual old shell. No movie-card save helper auto-load. */
(function(){
'use strict';
const VERSION='V6.24 Legacy Shell Bridge -> V7.12.176 Group Play Route Truth';
const DEF='test-page';
...
const FOUNDATION='v7-12-176-group-play-route-truth';
const R={
...
function loadScript(src){try{let base=src.split('?')[0];if(Array.from(document.scripts||[]).some(s=>String(s.src||'').includes(base)))return;let s=document.createElement('script');s.src=src;s.defer=true;s.dataset.sbLoadedBy='legacy-shell-bridge-safe';document.head.appendChild(s);}catch(e){}}
...
function hideLegacyShellArtifacts(){try{document.querySelectorAll('.sb-shell-menu-toggle,.sb-shell-scrim,.sb-shell-drawer,.sb104-right,.sb104-icons,.sb104-account,.sb104-profile,.sb104-search,.sb104-panel,#sbSharedShellMenu,#sbSharedShellDrawer,#sbSharedShellScrim').forEach(el=>{el.remove();});}catch(e){}try{document.querySelectorAll('style#sbSharedShellStyle').forEach(el=>el.remove());}catch(e){}document.documentElement.dataset.sbLegacyVisualShellRetired='true';}
function patchRoutes(root){root=root||document;try{root.querySelectorAll('a[href],form[action],[data-href],[data-target],[data-route],[data-url]').forEach(el=>{['href','action','data-href','data-target','data-route','data-url'].forEach(attr=>{let old=el.getAttribute(attr),fix=fixedUrl(old);if(old&&fix&&fix!==old){el.setAttribute(attr,fix);el.dataset.sbRouteFixedBy='legacy-shell-bridge-v7-12-176';}});});}catch(e){}}
function expose(){window.StreamBanditRoutes=Object.assign(window.StreamBanditRoutes||{},R,{groupPlayer:R.player2});window.StreamBanditSupabaseConfig={url:SUPABASE_URL,key:SUPABASE_KEY,source:'stream-bandit-shell-v6-24.js'};window.StreamBanditShellConfig=window.StreamBanditSupabaseConfig;window.SUPABASE_URL=window.SUPABASE_URL||SUPABASE_URL;window.SUPABASE_KEY=window.SUPABASE_KEY||SUPABASE_KEY;}
function ensureFoundation(){loadScript('stream-bandit-theme-projector-v7-12-156.js?v='+FOUNDATION);loadScript('stream-bandit-header-shell-v7-12-156.js?v='+FOUNDATION);loadScript('stream-bandit-footer-shell-v7-12-156.js?v='+FOUNDATION);loadScript('stream-bandit-global-helper-loader-v7-12-126.js?v='+FOUNDATION);loadScript('live-readiness-search-supabase-fallback-v7-12-130.js?v='+FOUNDATION);loadScript('stream-bandit-core-saves-v6-75.js?v='+FOUNDATION);}
function refresh(){expose();setFavicon();hideLegacyShellArtifacts();patchRoutes(document);ensureFoundation();try{if(window.StreamBanditThemeProjector&&window.StreamBanditThemeProjector.refresh)window.StreamBanditThemeProjector.refresh();}catch(e){}return state();}
function state(){return{version:VERSION,visualShellRetired:true,foundation:true,movieCardHelperAutoLoad:false,routes:R,supabase:{url:SUPABASE_URL,keyPresent:!!SUPABASE_KEY}};}
function boot(){refresh();setTimeout(refresh,200);setTimeout(refresh,800);setTimeout(refresh,1800);setInterval(function(){hideLegacyShellArtifacts();patchRoutes(document);ensureFoundation();},3000);window.StreamBanditShell={version:VERSION,routes:R,refresh,state,config:()=>({url:SUPABASE_URL,key:SUPABASE_KEY})};window.StreamBanditLegacyShellBridge=window.StreamBanditShell;document.documentElement.dataset.sbLegacyShellBridge='v7-12-176-group-play-route-truth';}
