/* Stream Bandit V7.4.3 Shared Shell / Menu Group Reorganisation
   Safe UI pass only: reorganises drawer groups and fixes quick-search movie Details route.
   No saves, uploads, deletes, billing, permission writes, database writes or live/index promotion. */
(function(){
'use strict';
const VERSION='V7.4.3 Shared Shell Menu Groups';
...
const GLOBAL_SEARCH='global-search-menu-upgrade-v6-27-test.html';
...
['🎨','Settings Studio','settings-studio-admin-shell-v6-55-test.html','Theme/global studio'],
['👤','Profile Settings','profile-settings-admin-shell-v6-56-test.html','Profile settings V7.0.4 route'],
['🏗️','Web Builder','web-builder-admin-shell-v6-57-test.html','Web builder current'],
['🧱','Platform Builder','platform-builder-admin-shell-v6-58-test.html','Platform builder current'],
...
function groupFor(p){const n=p[1];
 ...
 if(['Settings','Settings Studio','Profile Settings','Web Builder','Platform Builder','Final Shell Navigation'].includes(n))return 'Settings';
...
async function init(){addStyle();injectDrawer();await loadMovies();const s=ensureSearch();if(s){s.input.addEventListener('input',()=>renderSearch(s.input,s.overlay),true);s.input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();fullSearch(s.input)}if(e.key==='Escape')s.overlay.classList.remove('open')},true);if(s.button)s.button.addEventListener('click',e=>{e.preventDefault();fullSearch(s.input)},true);const close=s.overlay.querySelector('#closeSearch');if(close)close.onclick=()=>s.overlay.classList.remove('open');document.addEventListener('click',e=>{if(!s.wrap.contains(e.target))s.overlay.classList.remove('open')})}document.documentElement.dataset.streamBanditShell='v7-4-3'}
