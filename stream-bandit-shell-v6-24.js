/* Stream Bandit V7.12.42 Shared Shell Clean Machine Menu
   Overlay menu + route guard. Passed clean pages promoted: Details, Favourites, Liked,
   Supabase Library, Route Guard and Route Pointer. No database writes, no live/index promotion. */
(function(){
'use strict';
const VERSION='V7.12.42 Shared Shell Clean Machine Menu';
...
const FAVOURITES='favourites-clean-machine-v7-12-41-test.html';
const LIKED='likes-clean-machine-v7-12-42-test.html';
...
const ROUTE_FIXES={
 'global-search-admin-shell-v6-52-test.html':GLOBAL_SEARCH,
 'global-search-v5-90-test.html':GLOBAL_SEARCH,
 'home-watch-shell-v6-32-test.html':'home-global-helpers-v7-4-4-test.html',
 'details-global-helpers-v7-3-0-test.html':CURRENT_DETAILS,
 'details-global-helpers-v7-3-1-test.html':CURRENT_DETAILS,
 'player-watch-shell-v6-34-test.html':CURRENT_PLAYER,
 ...
 'favourites-watch-shell-v6-38-test.html':FAVOURITES,
 'favourites-global-helpers-v7-3-6-test.html':FAVOURITES,
 'liked-watch-shell-v6-39-test.html':LIKED,
 'likes-global-helpers-v7-3-7-test.html':LIKED,
 ...
 'platform-builder-admin-shell-v6-58-test.html':CLEAN_MENU,
 'platform-control-tower-live-machine-route-doctor-v7-12-32-test.html':CLEAN_MENU,
 'platform-control-tower-machine-gated-route-doctor-v7-12-31-test.html':CLEAN_MENU,
 'platform-control-tower-route-doctor-v7-12-30-test.html':CLEAN_MENU,
 'platform-control-tower-route-doctor-v7-12-29-test.html':CLEAN_MENU,
...
const pages=[
 ...
 ['🎬','Details',CURRENT_DETAILS,'Clean Details passed'],
 ['▶️','Player 1',CURRENT_PLAYER,'Single-title Player 1'],
 ...
 ['⭐','Favourites',FAVOURITES,'Clean Favourites passed'],
 ['👍','Liked',LIKED,'Clean Liked passed'],
 ...
 ['🔎','Global Search',GLOBAL_SEARCH,'Global search results'],
 ...
 ['🧱','Clean Machine Menu',CLEAN_MENU,'Passed machines plus route tools'],
 ['🛡️','Route Guard Proof',ROUTE_GUARD,'End-of-batch scanner'],
 ['🎯','Route Pointer Machine',ROUTE_POINTER,'One-page route picker'],
...
function applyFavicon(){try{const url=FAVICON_URL+(FAVICON_URL.includes('?')?'&':'?')+'sbFav='+Date.now();['icon','shortcut icon','apple-touch-icon'].forEach(rel=>{let l=document.querySelector('link[rel="'+rel+'"]');if(!l){l=document.createElement('link');l.rel=rel;document.head.appendChild(l);}l.href=url;});document.documentElement.dataset.streamBanditFavicon='loaded';}catch(e){}}
function fixedRouteValue(v){if(!v||typeof v!=='string'||!v.includes('.html'))return v;try{const u=new URL(v,location.href);const file=u.pathname.split('/').pop().split('#')[0];if(ROUTE_FIXES[file]){u.pathname=u.pathname.replace(file,ROUTE_FIXES[file]);return u.pathname.split('/').pop()+u.search+u.hash;}}catch(e){}const m=v.match(/([^\/?#]+\.html)(.*)$/);if(m&&ROUTE_FIXES[m[1]])return ROUTE_FIXES[m[1]]+(m[2]||'');return v;}
function applyRouteGuard(root=document){try{root.querySelectorAll('a[href],form[action],[data-href],[data-target],[data-route],[data-url]').forEach(el=>{['href','action','data-href','data-target','data-route','data-url'].forEach(attr=>{const val=el.getAttribute(attr);if(!val)return;const fixed=fixedRouteValue(val);if(fixed!==val){el.setAttribute(attr,fixed);el.dataset.sbRouteGuard='fixed';}});});document.documentElement.dataset.streamBanditRouteGuard='v7-12-42';}catch(e){}}
function guardClick(e){const a=e.target&&e.target.closest&&e.target.closest('a[href]');if(!a)return;const val=a.getAttribute('href'),fixed=fixedRouteValue(val);if(fixed&&fixed!==val){e.preventDefault();location.href=fixed;}}
